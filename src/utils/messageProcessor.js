...
class MessageProcessor {
  ...
  process(topic, payload) {
    ...
    if (_topic.type === Constants.Topic.REGISTRATION) {
      ...
      log.info({eventType : 'emit', topic : _topic.toString(), listener : Constants.DeviceEvents.DEVICE_SUBSCRIPTION, data : json});
    ...
    if (_topic.type === Constants.Topic.DEVICE && _topic.action === Constants.Topic.DEVICE_REQUEST) {
      log.info({eventType : 'emit', topic : _topic.toString(), data : json, listener : Constants.DeviceEvents.DEVICE_REQUEST});
      log.info({eventType : 'emit', topic : _topic.toString(), listener : `${Constants.DeviceEvents.DEVICE_REQUEST}/${json.id}`, data : json});
    ...
    if (_topic.type === Constants.Topic.DEVICE && _topic.action === Constants.Topic.DEVICE_COMMAND) {
      try {
        ...
        log.info({eventType : 'emit', topic : _topic.toString(), listener : Constants.DeviceEvents.DEVICE_COMMAND, data : json});
